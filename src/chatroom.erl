...
do_propegate_message(State, Ref, ClientPID, Message) ->
	...
	CliName = maps:get(ClientPID, State#chat_st.registrations),
	lists:foreach(fun (CliPID) -> CliPID!{request, self(), Ref, {incoming_msg, CliName, State#chat_st.name, Message}} end, maps:keys(maps:remove(ClientPID, State#chat_st.registrations))),
	NewState = #chat_st{
						name = State#chat_st.name,
						registrations = State#chat_st.registrations,
						history = State#chat_st.history++[{CliName, Message}]
						},
    NewState.
